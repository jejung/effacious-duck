...
public class FileBTree {
	 ...
	 * 
	 */
	public FileBTree(File file) throws IOException {
	 ...
	 * 
	 */
	public FileBTree(int order, File indexFile) throws IOException {
		this.order = order;
		this.maxKeys = order * 2;
		this.ioManager = new IOBlockManager(indexFile, this.maxKeys);
		this.root = this.ioManager.read(IOBlockManager.HEADER_SIZE);
	}
	...
	public boolean insert(String w) throws IOException {
		Key key = new Key();
		key.word = w;
		key.dataPos = this.search(this.root, key);
		if (key.dataPos == -1) {
			this.insert(key);
			return true;
		}
	...
	public static class IOBlockManager {
		 ...
		 * @param dataFile
		 */
		public IOBlockManager(File file, int keyCount) {
			super();
			this.file = file;
	...
	private static class Key implements Comparable<Key> {
		public static final int WORD_BYTES = 60;
		private String word;
		@Deprecated
